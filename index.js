...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Hanan-Algifri\n' // GANTI NAMA LU
            + 'ORG: SENSEI;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6288289824963 :+62 882-2263-3957\n' // GANTI NOMOR KAU
            + 'END:VCARD'
...
// POWERED BY HANAN ALGIRRI
prefix = "S"
blocked = []   
limitawal = 9999999999
memberlimit = 9999999999
cr = "*Artxym*"
...
// ANTI LINK GRUP
		        ...
		        setTimeout( () => {
			        baby.groupRemove(from, [kic]).catch((e)=>{reply(`BOT HARUS JADI ADMIN YAK!`)})
		        }, 3000)
		        setTimeout( () => {
			        baby.updatePresence(from, Presence.composing)
			        reply("Hedsot :v")
...
*/
				...
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix} SUBSCRIBE Art Bae_`)
				laysha = body.slice(8)
...
// PREMIUM
		...
		anu = await fetchJson(`https://api.vhtear.com/ytdl?link=${args[0]}&apikey=${VhtearKey}`, {method: 'get'})
					...
					break
				case 'tiktod':
					if (args.length < 1) return reply('Urlnya mana Bre?')
                  if (isBanned) return reply(ind.baned())
				if (!isRegistered) return reply(ind.noregis())
				if (!isPrem) return reply(ind.premium())
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply('URL NYA TIDAK VALID KAK')
...
*/
                ...
                addKoinUser('6288222633957@s.whatsapp.net', fee)
...
*/				
            	case 'mining':
                      ...
                      reply(`karena Arif Baik aku memberikan ${one}Xp >_<`)
                      }else{
            ...
            case 'owner':
            case 'creator':
                  baby.sendMessage(from, {displayname: "Arif", vcard: vcard}, MessageType.contact, { quoted: mek})
